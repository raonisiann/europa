...
/*
EuObject *expr_sum(EuObject *l, EuObject *r){
...
void value_eval(EuObject *v){
    if(v != NULL){
        printf("%s\n", v->__print(v));
        /*
        switch(v->type){
            case e_object:
                // use for casting? ((EuInt *)v->object)->base.class_name 
                printf("%s", v->object->__print(v->object));
            case e_string:
                if(v->size > 0){
                    EUROPA_OUTPUT("%s\n", v->str);
                }else{
...
void return_eval(struct ast_node *n, struct e_context *ctxt){
    EuObject *ret_val = expr_eval(n, ctxt);
        ...
        */
        case node_token:
            switch(n->token->type){
                ...
                */
                case plus:
                    return ((EuInteger *)l)->__sum(l, r);
                break;
                case minus:
                    return ((EuInteger *)l)->__sub(l, r);
                break;
                case multiply:
                    return ((EuInteger *)l)->__multiply(l, r);
                break;
                case division:
                    return ((EuInteger *)l)->__divide(l, r);
                break;
            }
            break;
    }
